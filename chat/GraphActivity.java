...
public class GraphActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i=0;i<id.length;i++){
            entry_chart.add(new Entry(id[i], temp[i]));
            entry2.add(new Entry(id[i], humi[i]));
            Log.e("LOG", "" +temp[i] + " " + humi[i]);
        }
...
//        entry_chart.add(new Entry(5, 10));
    ...
    기존 (2, 3)이 사라지는게 아니라 x가 2인곳에 y가 3, 5의 점이 찍힘 */
        ...
        databaseReference.child("chat").child(DATA_NAME).addChildEventListener(new ChildEventListener() {  // message는 child의 이벤트를 수신합니다.
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                ...
                Log.e("LOG", "" + index);
                entry_chart.add(new Entry(index,Data.getHumi()));
                Log.e("LOG", "" +index + " " + humi[index-1]);
                entry2.add(new Entry(index, Data.getTemp()));
                Log.e("LOG", "" +index + " " + temp[index-1]);

                Log.e("LOG", "후엥 그래프 표시했져");
    ...
    private void drawData() {
        LineDataSet lineDataSet = new LineDataSet(entry_chart, "꺽었다");
